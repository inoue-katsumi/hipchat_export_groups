...
help_message = '''
A simple script to export group messages from HipChat using the v2 API
...
Options:
  ...
  -l, --list          List the active rooms that will be queried
  ...
  -b                  Export messages older than this epochtime

Examples:

  extract every group messages:

  hipchat_export.py --user_token jKHxU8x6Jj25rTYaMuf6yTe7YpQ6TV413EUkBd0Z

  extract only messages in certain groups:
...
working directory, and folders will be created for each group it will ask
for group chat history (this list is determined by a dictionary in the main()
...
def get_user_list(user_token):
    ...
    # url = HIPCHAT_API_URL + "/user?start-index=180&max-results=1000"
    url = HIPCHAT_API_URL + "/room?start-index=2471&max-results=1000"
    ...
    # Iterate through the groups and make a dict to return
...
def display_userlist(user_list):
    print("\nThe following groups are active and will be queried for group messages:\n")
...
def message_export(user_token, user_id, user_name):
    ...
    global HIPCHAT_API_URL
    #url = HIPCHAT_API_URL + "/room/%s/history?date=%s&reverse=false&max-results=1000&end-date=2016-01-01T00:00:00%%2B00:00" % (user_id, int(time())-5)
    #url = HIPCHAT_API_URL + "/room/%s/history?date=%s&reverse=false&end-date=1451606400" % (user_id, int(time())-5)

    # Human time (GMT/UTC): Saturday, July 1, 2017 12:00:00 AM
    #url = HIPCHAT_API_URL + "/room/%s/history?date=%s&reverse=false&end-date=1498867200" % (user_id, int(time())-5)
    # Human time (GMT/UTC): April 1, 2018 12:00:00 AM for 1 year, former number  is Saturday, April 1, 2017 12:00:00 AM
    #url = HIPCHAT_API_URL + "/room/%s/history?date=%s&reverse=false&end-date=1491004800" % (user_id, 1522540800)
    url = HIPCHAT_API_URL + "/room/%s/history?date=%s&reverse=false&end-date=1491004800" % (user_id, BEFORE_EPOCH)
...
def main(argv=None):
    ...
    try:
        try:
            opts, args = getopt.getopt(argv[1:], "hlmu:x:vb:",
        ...
        # Iterate through user list and export all group messages to disk
        if USER_SUBSET:
        ...
        for user_id, user_name in extract:
            log("\nExporting group messages for %s (ID: %s)..." % (user_name, user_id))
            try:
